...
module.exports = function modelBase (bookshelf, params) {
  ...
  var model = bookshelf.Model.extend({
    constructor: function () {
      ...
      var Model = bookshelf.model(this.modelName);

      Checkit.Validator.prototype.uniqueness = function(val, field, scope) {
        var query = {}
        query[field] = val
        ...
        return Model.query({ where: query }).fetch()
          .then(exists => {
            if (exists) throw new Checkit.ValidationError(`Record already exists for ${scope}`);
    ...
    validateSave: function (model, attrs, options) {
      return new Checkit(this.validate).run(this.attributes, this);
    }
