...
module.exports = function modelBase (bookshelf, params) {
  ...
  var model = bookshelf.Model.extend({
    constructor: function () {
      ...
      if (this.validate) {
        this.validator = new Checkit(this.validate)
    ...
    validateSave: function (model, attrs, options) {
      return this.validator.run(this.attributes, this);
    }
...
Checkit.Validator.prototype.uniqueness = function(val, scope) {
  const [ record, field ] = [...arguments].slice(-2)
  ...
  if (typeof scope === 'string') query[scope] = this._target[scope]

  return model.query({ where: query }).fetch()
    .then(exists => {
      if (exists) throw new Checkit.ValidationError(`Record already exists`);
